...
## The Model
Using this collection of sentences, I built a bi-gram Markov Model - the probability 
of the word depends on the bigram preceding it. Here's an example of an actual sentence
generated by the bot: "My hair may not be reliable", where the seed is "hair": 

![Markov Chain](https://github.com/araval/trump-bot/blob/master/images/markovChain.png)
...
The model requires a "seed" to generate a sentence. If the input does not 
...
project (GloVe), which were trained on Wikipedia and gigaword5. I wrote a C++ program 
(sklearn's knn was too slow) to find 10 most similar words for all 400,000 words. 
Here's an example of similar words from my program: 

![Similar words](https://github.com/araval/trump-bot/blob/master/images/words.png)

Now, if input does not contain a word in Trump's vocabulary, the bot searches for the  
similar words, and generates a sentence as soon as one is found. 
If none of the similar words are in the vocabulary, then model resorts to uttering a 
sentence from a list of canned responses, such as 
"Don't waste my time, I make a lot of money." or "You're fired!". 
